...
async def button(bot, update: CallbackQuery):
    ...
    if cb_data.startswith("cancel"):
        ...
        if update.from_user.id in AUTH_CHANNEL:
            g_d_list = ['extract', 'venv', 'rclone.conf', 'rclone_bak.conf', '.gitignore', '_config.yml', 'run', 'Dockerfile', 'Procfile', 'requirements.txt', 'runtime.txt', 'start.sh', 'tobrot', '.gtam', 'logs.txt', 'vendor', 'LeechBot.session', 'dht.dat', 'dht6.dat', '.setup.sh', '.aria2.conf', 'LeechBot.session-journal']

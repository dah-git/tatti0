...
from tobrot.plugins.incoming_message_fn import rename_tg_file, incoming_message_f, incoming_youtube_dl_f, incoming_purge_message_f, g_yt_playlist, upload_as_doc, upload_as_stream
from tobrot.plugins.rclone_size import g_clearme
...
if __name__ == "__main__" :
    ...
    #
    incoming_upload_as_doc = MessageHandler(
        upload_as_doc,
        filters=filters.command(
            ["doc"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_upload_as_doc)
    #
    incoming_upload_as_stream = MessageHandler(
        upload_as_stream,
        filters=filters.command(
            ["stream"]) & filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(incoming_upload_as_stream)
    #
    ...
    incoming_g_clear_handler = MessageHandler(
        g_clearme,
        filters=filters.command([f"{RENEWME_COMMAND}"]) & filters.chat(chats=OWNER_ID)
    )
    ...
    status_message_handler = MessageHandler(
        status_message_f,
        filters=filters.command([f"{STATUS_COMMAND}"]) & filters.chat(chats=OWNER_ID)
    )
    ...
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=filters.command(["exec"]) & filters.chat(chats=OWNER_ID)
    )
    ...
    eval_message_handler = MessageHandler(
        eval_message_f,
        filters=filters.command(["eval"]) & filters.chat(chats=OWNER_ID)
    )
    ...
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=filters.command([f"{UPLOAD_COMMAND}"]) & filters.chat(chats=OWNER_ID)
    )
    app.add_handler(upload_document_handler)
    #
    upload_log_handler = MessageHandler(
        upload_log_file,
        filters=filters.command([f"{LOG_COMMAND}"]) & filters.chat(chats=OWNER_ID)
    )
